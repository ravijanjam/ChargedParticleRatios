...
void pPbInvYield(char *inFile){
	...
	for (int tBin=0; tBin < 1000; tBin++)
	{
		...
		nPosBins[tBin] = pT;
		if (pT > 0) gPosInvYield[tBin] = dN /(dpT*pT*nEventspPb);
		...
		nNegBins[tBin] = pT;
		if (pT > 0) gNegInvYield[tBin] = dN /(dpT*pT*nEventspPb);
	...
	cInvYield->cd(1);
	graphPosInvYield->GetYaxis()->SetRangeUser(1e-7, 1e3);
	graphPosInvYield->Draw();

	cInvYield->cd(2);
	graphNegInvYield->GetYaxis()->SetRangeUser(1e-7, 1e3);
	...
	TFile *outputFile = new TFile("outputpPbInvariantYield.root", "recreate");
