...
void TestAnalyzer_v3::initHistos(const edm::Service<TFileService> & fs)
{
	...
	// Track histograms
	trackPosPt = fs->make<TH1F>("trackPosPt","Pt of positively charged tracks",100, 0, 300);
	trackPosEta = fs->make<TH1F>("trackPosEta","Eta of positively charged tracks",100, -5, 5 );

	trackNegPt = fs->make<TH1F>("trackNegPt","Pt of positively charged tracks",1000,0,300);
	trackNegEta = fs->make<TH1F>("trackNegEta","Eta of positively charged tracks",100, -5, 5);

	// For all tracks
	trackEta = fs->make<TH1F>("trackEta","Eta of all charged tracks",1000,-5, -5);
	trackpT = fs->make<TH1F>("trackpT","pT of all charged tracks",100, 0, 300);
