...
//   for(  track = tcol->begin(); track != tcol->end() && iVertexCuts ; ++track )
   ...
   { 
	...
	cout << "Track Quality: " << track->quality(reco::TrackBase::qualityByName("highPurity")) << endl;
...
bool
testAnalyzer::vtxSort( const reco::Vertex &  a, const reco::Vertex & b )
{
	return 0 /*Change here*/;
}

void testAnalyzer::initHistos(const edm::Service<TFileService> & fs)
{
...
bool
testAnalyzer::passesTrackCuts(const reco::Track & track, const reco::Vertex & vertex){
